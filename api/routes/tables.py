...
@fill_bp.route("/add_entity", methods=["POST"])
def add_entity():
    try:
...
@fill_bp.route("/delete_entity", methods=["POST"])
def delete_entity():
    try:
...
@fill_bp.route("/add_category", methods=["POST"])
def add_category():
    try:
        data = request.get_json()
        name = data.get("name", "").strip().lower()
...
@fill_bp.route("/delete_category", methods=["POST"])
def delete_category():
    try:
...
@fill_bp.route("/add_page", methods=["POST"])
def add_page():
    try:
        data = request.get_json()
...
@fill_bp.route("/delete_page", methods=["POST"])
def delete_page():
    try:
        id = request.json.get("id")
        if not id:
...
@fill_bp.route("/add_influence", methods=["POST"])
def add_influence():
    try:
        data = request.get_json()
